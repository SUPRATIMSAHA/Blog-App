...
router.post("/user/signup", async (req, res) => {
  try {
    ...
    let mailOptions = {
      from: "no.reply.blogapp21@gmail.com",
      to: newUser.email,
...
router.get("/reset/password", async (req, res) => {
  res.render("authentication/resetpassword", { email: req.query.email });
});
...
router.post("/reset/password", async (req, res) => {
  try {
    if (req.body.password !== req.body.confirm_password) {
      ...
      } else if (req.user && req.user.google.googleId) {
        res.redirect(`/user/${req.user.google.googleId}`);
      } else {
        res.redirect("/user/signin");
      }
    }
